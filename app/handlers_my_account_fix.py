...
async def handle_my_account(query, user, admin=False):
    ...
    # Mostrar estado del trial / expiración si existe en BD
    trial_until = user.get("trial_until")
    plan_until = user.get("plan_until")
    referred_by = user.get("referred_by")

    if trial_until:
        message += f"Trial hasta: {trial_until}\n"
    if plan_until:
        message += f"Vence: {plan_until}\n"

    message += f"Admin: {'Sí' if admin else 'No'}\n"

    if referred_by:
        message += f"Referido por: {referred_by}\n"

    message += "\nℹ️ La activación manual de planes se realiza solo desde el Panel Admin."

    # Mi Cuenta queda informativa; ya no muestra activar plan aquí
    reply_markup = InlineKeyboardMarkup([
        [InlineKeyboardButton("⬅️ Volver", callback_data="back_menu")],
    ])
