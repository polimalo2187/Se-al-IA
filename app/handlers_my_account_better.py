...
async def handle_my_account(query, user, admin=False):
    ...
    if plan_until:
        ...
        try:
            vence_date = datetime.strptime(str(plan_until)[:10], "%Y-%m-%d").date()
        ...
        estado = "Trial"
        try:
            vence_date = datetime.strptime(str(trial_until)[:10], "%Y-%m-%d").date()
            dias_restantes = (vence_date - today).days
        except Exception:
            pass
    ...
    try:
        stats = get_user_referral_stats(user["user_id"]) or {}
        ref_validos = int(stats.get("valid_referrals", 0) or 0)
        ...
        if ref_validos < 2:
            ...
            proxima_recompensa = "✅ Ya alcanzaste la recompensa máxima"
    except Exception:
        pass
    ...
    if dias_restantes is not None:
        lines.append(f"Días restantes: {dias_restantes}")

    lines.extend([
        "",
        f"👥 Referidos válidos: {ref_validos}",
