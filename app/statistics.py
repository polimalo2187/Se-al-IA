...
def _calculate_stats(from_date: datetime) -> Dict:
    # Antes de leer stats, evaluar señales expiradas pendientes
...
def _calculate_stats_by_plan(from_date: datetime, plan: str) -> Dict:
    """
    Estadísticas por visibilidad del plan:
    - free
    - plus
    - premium
    """
...
def get_last_days_stats_by_plan(days: int) -> Dict:
    """
    Retorna estadísticas separadas por plan.
    """
    now = datetime.utcnow()
    from_date = now - timedelta(days=days)
...
def get_signal_activity_stats_by_plan(days: int) -> Dict:
    """
    Actividad del scanner separada por visibilidad del plan.
    """
    now = datetime.utcnow()
    from_date = now - timedelta(days=days)
