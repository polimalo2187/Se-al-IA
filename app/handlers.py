...
Activa tu plan para ver tu historial de señales.",
            reply_markup=InlineKeyboardMarkup([
    ...
    try:
        cur = user_signals_collection().find(
            {"user_id": user["user_id"]},
            {
                "symbol": 1,
                "direction": 1,
...
Aún no tienes señales registradas en tu historial.",
    ...
    now = datetime.utcnow()
    lines = ["🧾 *HISTORIAL DE SEÑALES*", "", f"Últimas {len(rows)} señales:", ""]

    for i, s in enumerate(rows, start=1):
        symbol = str(s.get("symbol", "—"))
        direction = str(s.get("direction", "—")).upper()
        fp = str(s.get("fingerprint", "—"))
        vis = str(s.get("visibility", plan)).upper()
        ...
        lines.append(f"   Plan: {vis} | Estado: {status}")
        lines.append(f"   Hora: `{created_str} UTC`")
...
def get_handlers():
    return [
        CallbackQueryHandler(
            handle_menu,
            pattern="^(view_signals|radar|radar_refresh|performance|movers|market|market_refresh|watchlist|wl_refresh|wl_clear|wl_rm:[A-Z0-9]+|alerts|history|history_refresh|plans|my_account|referrals|support|admin_panel|admin_activate_plan|admin_delete_user|back_menu|choose_plus_plan|choose_premium_plan|register_exchange)$"
        ),
