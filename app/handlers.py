...
async def handle_performance(query, user, admin):
    ...
    # Stats reales basadas en signal_results (si existen) + actividad de señales.
    ...
    def _fmt_block(title: str, s: dict, a: dict) -> str:
        ...
        if s.get("total", 0) > 0:
            lines.append(f"• Evaluadas: {s['total']}  | ✅ {s['won']}  | ❌ {s['lost']}  | ⏳ {s['expired']}")
        ...
        lines.append(f"• Señales generadas: {a.get('signals_total', 0)} | Score prom.: {a.get('avg_score', '—')}")
        return "
".join(lines)
...
".join(out)
    ...
    msg += _fmt_score("Win rate por score (7D)", score_7d) + "

"
    msg += _fmt_score("Win rate por score (30D)", score_30d)

    await query.edit_message_text(msg, reply_markup=back_to_menu(), parse_mode="Markdown")
