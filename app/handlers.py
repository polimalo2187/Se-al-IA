...
async def handle_reset_stats(query, user):
    ...
    try:
        summary = reset_statistics(preserve_signals=True) if callable(reset_statistics) else None

        if isinstance(summary, dict):
            deleted_results = summary.get("deleted_results", 0)
            reset_base = summary.get("reset_base_signals", 0)
            reset_user = summary.get("reset_user_signals", 0)
            message = (
                "♻️ Estadísticas restablecidas correctamente.\n\n"
                f"• Resultados borrados: {deleted_results}\n"
                f"• Señales base rearmadas para reevaluación: {reset_base}\n"
                f"• Señales usuario limpiadas: {reset_user}\n\n"
                "El histórico bruto del scanner se conserva."
            )
        else:
            message = (
                "♻️ Estadísticas restablecidas correctamente.\n\n"
                "Se limpiaron los resultados evaluados y se conservó el histórico bruto del scanner."
            )
