...
from app.database import users_collection, user_signals_collection
...
async def handle_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if action == "admin_panel" and admin:
            await query.edit_message_text(
                ...
                reply_markup=InlineKeyboardMarkup([
                    ...
                    [InlineKeyboardButton("🔎 Buscar usuario por ID señal", callback_data="admin_find_signal_user")],
        ...
        if action == "admin_find_signal_user" and admin:
            context.user_data["awaiting_signal_fingerprint"] = True
            await query.edit_message_text("🔎 Envía el ID de la señal que deseas rastrear:")
            return
...
async def handle_text_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if (
        ...
        or context.user_data.get("awaiting_signal_fingerprint")
    ):
...
async def handle_performance(query, user):
    ...
    def _fmt_plan_block(title: str, s: dict, a: dict | None = None) -> str:
        total = s.get("total", 0)
        won = s.get("won", 0)
        lost = s.get("lost", 0)
        expired = s.get("expired", 0)
        ...
        if a:
            rows.append(f"• Señales scanner: {a.get('signals_total', 0)} | Score prom: {a.get('avg_score', '—')}")
    ...
    if by_plan_30:
        ...
        parts.append(_fmt_plan_block("🟢 FREE", (by_plan_30 or {}).get("free", {}), (act_by_plan_30 or {}).get("free")))
        parts.append("")
        parts.append(_fmt_plan_block("🟡 PLUS", (by_plan_30 or {}).get("plus", {}), (act_by_plan_30 or {}).get("plus")))
        parts.append("")
        parts.append(_fmt_plan_block("🔴 PREMIUM", (by_plan_30 or {}).get("premium", {}), (act_by_plan_30 or {}).get("premium")))

    if act7 or act30:
        parts.append("\n📈 **Actividad de señales (scanner)**")
        if act7:
...
async def handle_admin_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if context.user_data.get("awaiting_signal_fingerprint"):
            fingerprint = target_user_id_str.strip()

            if not is_admin(update.effective_user.id):
                await update.message.reply_text("❌ Permisos revocados.")
                context.user_data["awaiting_signal_fingerprint"] = False
                return

            signals_col = user_signals_collection()
            loop = asyncio.get_event_loop()

            user_signal = await loop.run_in_executor(
                None,
                lambda: signals_col.find_one({"fingerprint": fingerprint})
            )

            context.user_data["awaiting_signal_fingerprint"] = False

            if not user_signal:
                await update.message.reply_text("❌ No encontré ninguna señal con ese ID.")
                return
            ...
            message = (
                ...
                f"ID señal: {fingerprint}\n"
                f"ID Telegram: {user_signal.get('user_id', '—')}\n"
                f"Par: {user_signal.get('symbol', '—')}\n"
                f"Dirección: {user_signal.get('direction', '—')}\n"
                f"Plan: {str(user_signal.get('visibility', '—')).upper()}\n"
                f"Fecha: {created_at_str}"
            )
...
def get_handlers():
    return [
        CallbackQueryHandler(
            handle_menu,
            pattern="^(view_signals|radar|radar_refresh|performance|reset_stats|movers|market|market_refresh|watchlist|wl_refresh|wl_clear|wl_rm:[A-Z0-9]+|alerts|alerts_refresh|history|history_refresh|plans|my_account|referrals|support|admin_panel|admin_activate_plan|admin_delete_user|admin_find_signal_user|back_menu|choose_plus_plan|choose_premium_plan|register_exchange)$"
        ),
