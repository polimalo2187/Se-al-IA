...
def get_handlers():
    return [
        CallbackQueryHandler(
            handle_menu,
            pattern="^(view_signals|plans|my_account|referrals|support|admin_panel|admin_activate_plan|register_exchange|back_menu|choose_plus_plan|choose_premium_plan|performance|radar|radar_refresh|movers|market|market_refresh|watchlist|wl_refresh|wl_clear|alerts|history)$"
        ),
        CallbackQueryHandler(handle_copy_ref_code, pattern="^copy_ref_code$"),
        CallbackQueryHandler(handle_menu, pattern="^wl_rm:.*$"),
...
async def handle_performance(query, user, admin):
    ...
    plan = _effective_plan(user, admin)
    if plan == PLAN_FREE:
        await query.edit_message_text(_locked_message("🎯 Rendimiento del bot", "PLUS / PREMIUM"), reply_markup=back_to_menu())
        return
    await query.edit_message_text("🎯 Rendimiento del bot\n\n✅ Próximamente (stats 7D/30D, winrate por score).", reply_markup=back_to_menu())
