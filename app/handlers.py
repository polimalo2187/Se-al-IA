...
async def handle_my_account(query, user, admin=False):
    ...
    if plan_until:
        vence = plan_until
        try:
            vence_date = datetime.strptime(plan_until, "%Y-%m-%d").date()
            dias_restantes = (vence_date - now).days
            if dias_restantes < 0:
                estado = "Vencido"
            else:
                estado = "Activo"
    ...
    try:
        ...
        ref_validos = stats.get("valid_referrals", 0)
    except:
        pass
...
e planes se realiza desde el Panel Admin."
