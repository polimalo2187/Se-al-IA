...
async def handle_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if action == "radar":
            await handle_locked_or_soon(query, user, feature="📡 Radar Futures", required_plan=PLAN_PLUS)
            return

        if action == "movers":
            await handle_locked_or_soon(query, user, feature="🔥 Movers", required_plan=PLAN_FREE)
            return
...
def get_handlers():
    return [
        CallbackQueryHandler(
            handle_menu,
            pattern="^(view_signals|radar|performance|movers|market|watchlist|alerts|history|plans|my_account|referrals|support|admin_panel|admin_activate_plan|back_menu|choose_plus_plan|choose_premium_plan|register_exchange)$"
        ),
