...
def generate_user_signal(base_signal: Dict, user_id: int) -> Dict:
    ...
    if direction == "LONG":
        sl_base = round(entry * 0.99, 4)
        tp1_base = round(entry * 1.01, 4)
        tp2_base = round(entry * 1.02, 4)
    else:
        sl_base = round(entry * 1.01, 4)
        tp1_base = round(entry * 0.99, 4)
        tp2_base = round(entry * 0.98, 4)
