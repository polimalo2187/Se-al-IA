...
def generate_user_signal(base_signal: Dict, user_id: int) -> Dict:
    ...
    if direction == "LONG":
        sl_base = round(entry * 0.988, 4)   # -1.2%
        tp1_base = round(entry * 1.01, 4)   # +1.0%
        tp2_base = round(entry * 1.02, 4)   # +2.0%
    else:
        sl_base = round(entry * 1.012, 4)   # +1.2%
        tp1_base = round(entry * 0.99, 4)   # -1.0%
        tp2_base = round(entry * 0.98, 4)   # -2.0%

    fixed_profiles = {
        p: {
            "stop_loss": sl_base,
            "take_profits": [tp1_base, tp2_base]
        }
...
# ======================================================
# EVALUACIÓN AUTOMÁTICA DE SEÑALES BASE (PERFIL CONSERVADOR)
# ======================================================
...
def _evaluate_base_signal_result(base_signal: Dict) -> str:
    """
    Reglas:
    - TP1 base = WON
    - SL base = LOST
    ...
    """
    direction = str(base_signal.get("direction", "")).upper()
    symbol = base_signal.get("symbol")
    stop_loss = base_signal.get("stop_loss")
    take_profits = base_signal.get("take_profits", [])
    tp1 = take_profits[0] if take_profits else None

    created_at = base_signal.get("created_at")
    valid_until = base_signal.get("valid_until")
...
def evaluate_expired_signals(limit: int = 100) -> int:
    """
    Evalúa señales BASE expiradas pendientes y registra un único resultado por señal base.
    Retorna cuántas señales base fueron evaluadas.
    """
    now = datetime.utcnow()
    pending = list(
        signals_collection()
    ...
    for s in pending:
        try:
            ...
            existing_result = signal_results_collection().find_one(
                {
                    "signal_id": signal_id,
            ...
            result = _evaluate_base_signal_result(s)
            ...
            result_doc = {
                ...
                "tp_used": s.get("take_profits", [None])[0],
