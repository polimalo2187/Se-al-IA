...
from app.watchlist import get_symbols as wl_get_symbols, add_symbol as wl_add_symbol, remove_symbol as wl_remove_symbol, clear as wl_clear, normalize_many as wl_normalize_many
from app.watchlist_ui import render_watchlist_view, CB_WL_REFRESH, CB_WL_CLEAR, CB_WL_REMOVE_PREFIX
...
async def handle_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if action == "watchlist":
        # Activar modo Watchlist para capturar texto del usuario
        context.user_data['watchlist_active'] = True
        # Guardar el último mensaje de watchlist para posibles ediciones futuras
        context.user_data['watchlist_msg_id'] = query.message.message_id if query.message else None
        context.user_data['watchlist_chat_id'] = query.message.chat_id if query.message else None
            plan = (user.get("plan") or PLAN_FREE)
            await handle_watchlist_view(query, user, plan)
            context.user_data["watchlist_mode"] = True
            return
        if action == CB_WL_REFRESH:
            plan = (user.get("plan") or PLAN_FREE)
            await handle_watchlist_refresh(query, user, plan)
            context.user_data["watchlist_mode"] = True
            return
        if action == CB_WL_CLEAR:
            await handle_watchlist_clear(query, user)
            context.user_data["watchlist_mode"] = True
            return
        if action.startswith(CB_WL_REMOVE_PREFIX):
            sym = action.split(":", 1)[1]
            await handle_watchlist_remove(query, user, sym)
            context.user_data["watchlist_mode"] = True
            return
...
async def handle_watchlist_view(query, user, plan: str):
    """
    ...
    - PREMIUM: ilimitado
    """
    symbols = wl_get_symbols(user["user_id"])
    text, kb = render_watchlist_view(symbols)
    # Guardamos estado para capturar texto y añadir símbolos
    # (Solo afecta cuando el usuario está en pantalla watchlist)
    try:
...
async def handle_watchlist_refresh(query, user, plan: str):
    # En esta versión, refresh solo re-renderiza (más adelante puede añadir precios/funding).
    await handle_watchlist_view(query, user, plan)


async def handle_watchlist_clear(query, user):
    wl_clear(user["user_id"])
    symbols = wl_get_symbols(user["user_id"])
    text, kb = render_watchlist_view(symbols)
    try:
...
async def handle_watchlist_remove(query, user, symbol: str):
    wl_remove_symbol(user["user_id"], symbol)
    symbols = wl_get_symbols(user["user_id"])
    text, kb = render_watchlist_view(symbols)
    try:
        await query.edit_message_text(f"✅ Eliminado: {symbol}\n\n" + text, reply_markup=kb)
    except BadRequest as e:
        if "Message is not modified" in str(e):
            return
        raise
...
async def handle_watchlist_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Captura texto mientras el usuario está en Watchlist y añade símbolos.
    Acepta: BTC, ETHUSDT, SOL/USDT, etc. (se normaliza a XXXUSDT)
    """
    user_id = update.effective_user.id
    users_col = users_collection()
    user = users_col.find_one({"user_id": user_id})
    if not user:
        await update.message.reply_text("❌ Usuario no encontrado. Usa /start.")
        return

    plan = user.get("plan") or PLAN_FREE

    raw = (update.message.text or "").strip()
    symbols = wl_normalize_many(raw)
    if not symbols:
        await update.message.reply_text("❌ No entendí el símbolo. Ej: BTCUSDT o BTC, ETH, SOL")
        return

    added = []
    blocked = []
    for s in symbols:
        res = wl_add_symbol(user_id, s, plan=plan)
        if res.ok and s in res.symbols:
            # si ya existía, res.ok True también; para diferenciar, chequeamos mensaje
            if "Añadido" in res.message:
                added.append(s)
        else:
            blocked.append(s)

        # si se alcanzó límite, paramos
        if "Límite alcanzado" in res.message:
            break

    # Re-render vista
    symbols_now = wl_get_symbols(user_id)
    text, kb = render_watchlist_view(symbols_now)

    prefix = ""
    if added:
        prefix += "✅ Añadidos: " + ", ".join(added) + "\n\n"
    if blocked:
        prefix += "🔒 No se pudieron añadir: " + ", ".join(blocked) + "\n\n"

    await update.message.reply_text(prefix + text, reply_markup=kb)
...
def get_handlers():
    return [
        CallbackQueryHandler(
            handle_menu,
            pattern="^(view_signals|radar|radar_refresh|performance|movers|market|watchlist|alerts|history|plans|my_account|referrals|support|admin_panel|admin_activate_plan|back_menu|choose_plus_plan|choose_premium_plan|register_exchange|wl_refresh|wl_clear|wl_rm:.*)$"
        ),
