...
def _compute_raw_score(df: pd.DataFrame, direction: str, profile: Dict, quality: Dict[str, float]) -> Tuple[float, List[Tuple[str, float]]]:
    ...
    raw_score = round(_clamp(raw_score, 0.0, MAX_SCORE), 2)
...
def _evaluate_profile(
    df: pd.DataFrame,
    profile: Dict,
) -> Optional[Tuple[str, float, float, List[Tuple[str, float]], Dict[str, Dict], str]]:
    last = df.iloc[-1]
...
def mtf_strategy(
    ...
    if shared_result:
        direction, entry_price, raw_score, components, trade_profiles, setup_group = shared_result
        return {
            "direction": direction,
    ...
    if free_result:
        direction, entry_price, raw_score, components, trade_profiles, setup_group = free_result
        return {
            "direction": direction,
            "entry_price": entry_price,
            "stop_loss": trade_profiles["conservador"]["stop_loss"],
