...
VOLUME_MULTIPLIER_BREAKOUT = 1.15
...
def breakout_confirmation(df: pd.DataFrame, direction: str) -> bool:
    ...
    if direction == "LONG":
        ...
        # evita entrar si la vela ya está demasiado extendida
        close_position = (last["high"] - last["close"]) / candle_range
...
def pullback_confirmation(df: pd.DataFrame, direction: str) -> bool:
    ...
    if direction == "LONG":
        ...
        # evita pullbacks que ya rebotaron demasiado
        distance = (last["close"] - last["bb_mid"]) / last["bb_mid"]
...
# =========================
# Nota: en esta versión el setup de señal es único.
# La separación FREE / PLUS / PREMIUM la hace scanner.py por ranking/score asignado,
# no por lógica distinta dentro de la estrategia.
...
def mtf_strategy(
    ...
    if is_breakout:
        score += 25
        components.append(("breakout_5m", 25))
    else:
        score += 20
    ...
    if df_1h.iloc[-1]["adx"] >= 22:
        score += 5
        components.append(("adx_bonus_1h", 5))

    if last_5m["volume"] >= last_5m["vol_ma"] * 1.15:
        score += 5
