# app/referrals.py - VERSIÓN ACTUALIZADA CON HADES_ALPHA_bot
...
def check_ref_rewards(referrer_id: int) -> bool:
    # Recompensas ahora se aplican automáticamente (+7 días por referido válido)
    return False
...
def get_referral_link(user_id: int) -> str:
    ...
    if not user:
        return f"https://t.me/HADES_ALPHA_bot?start=ref_{user_id}"
    
    ref_code = user.get("ref_code", f"ref_{user_id}")
    return f"https://t.me/HADES_ALPHA_bot?start={ref_code}"
