...
async def handle_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if action == "market":
            try:
                from app.market_ui import render_market
                text_market, kb_market = render_market()
                await query.edit_message_text(text_market, reply_markup=kb_market)
            except BadRequest as e:
                if "Message is not modified" not in str(e):
                    raise
            except Exception:
                logging.exception("Market open error")
                await query.edit_message_text(
                    "❌ No pude abrir Mercado ahora mismo.",
        ...
        if action == "market_refresh":
            try:
                ...
                logging.exception("Market refresh error")
                await query.answer("❌ No pude actualizar Mercado.", show_alert=False)
            return
...
async def handle_my_account(query, user, admin=False):
    ...
    if plan_until:
        vence = str(plan_until)
        vence_date = _parse_date(plan_until)
        if vence_date:
            dias_restantes = (vence_date - now_date).days
            estado = "Activo" if dias_restantes >= 0 else "Vencido"
    elif trial_until:
        vence = str(trial_until)
        estado = "Trial"
        vence_date = _parse_date(trial_until)
        if vence_date:
            dias_restantes = (vence_date - now_date).days
            if dias_restantes < 0:
    ...
    ref_validos = 0
    proxima_recompensa = "2 referidos → 7 días Premium"
    try:
        stats = get_user_referral_stats(user["user_id"])
        ref_validos = int(stats.get("valid_referrals", 0))
    except Exception:
        ref_validos = 0
    ...
    if ref_validos < 2:
        ...
        proxima_recompensa = "5 referidos → 30 días Premium"
    elif ref_validos < 20:
        proxima_recompensa = "20 referidos → Premium de por vida"
    else:
        proxima_recompensa = "✅ Ya alcanzaste el máximo de recompensas"
...
"
    ...
    if vence:
        message += f"Vence: {vence}
"

    if dias_restantes is not None:
        message += f"Días restantes: {dias_restantes}
"

    message += (
        f"
👥 Referidos válidos: {ref_validos}
"
        f"Próxima recompensa: {proxima_recompensa}
"
        "
ℹ️ La activación de planes se realiza desde el Panel Admin."
    )
