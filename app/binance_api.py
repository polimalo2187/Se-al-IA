...
def _get_klines(symbol: str, interval: str, limit: int = 2) -> List[List[Any]]:
    """Obtiene velas de Binance Futures con cache."""
    symbol = symbol.upper().strip()
...
def get_radar_opportunities(limit: int = 10) -> List[Dict[str, Any]]:
    ...
    - score usando:
        ...
        30% volumen 24h
        20% trades 24h
    - momentum textual usando 15m + 1h
    ...
    for t in tickers:
        ...
        rows.append(
            {
                "symbol": symbol,
                "change_pct": round(change_1h, 2),  # compatibilidad con handlers
    ...
    changes_1h = [r["abs_change_pct"] for r in rows]
    ...
    for i, r in enumerate(rows):
        combined = (0.50 * r_1h[i]) + (0.30 * r_vol[i]) + (0.20 * r_trd[i])
    ...
    for r in selected:
        ...
        r["priceChangePercent"] = r["change_pct"]  # compatibilidad
