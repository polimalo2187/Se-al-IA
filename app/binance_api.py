...
_TTL_KLINES = 45           # velas 15m / 1h para radar
...
def _get_klines(symbol: str, interval: str, limit: int = 2) -> List[List[Any]]:
    """Velas por símbolo e intervalo (cached)."""
    symbol = symbol.upper().strip()
...
def _pct_change_from_last_candle(symbol: str, interval: str) -> float:
    """
    % de cambio de la última vela cerrada:
    (close - open) / open * 100
    """
...
def get_radar_opportunities(limit: int = 10) -> List[Dict[str, Any]]:
    ...
    - score usando 1h real + volumen 24h + trades 24h
    - momentum textual usando 15m y 1h
    ...
    for t in tickers:
        ...
        change_1h = _pct_change_from_last_candle(symbol, "1h")
        change_15m = _pct_change_from_last_candle(symbol, "15m")

        rows.append(
            {
                "symbol": symbol,
                "change_pct": round(change_1h, 2),   # ESTE ES EL CAMBIO QUE MOSTRARÁ EL RADAR
                "change_15m": round(change_15m, 2),
                "change_1h": round(change_1h, 2),
                "abs_change_pct": abs(change_1h),
    ...
    for i, r in enumerate(rows):
        # Más peso a 1h real, sin romper la liquidez/actividad
        ...
        # Momentum usando 15m + 1h
        strength = (abs(r["change_15m"]) * 0.6) + (abs(r["change_1h"]) * 0.4)
        if strength >= 5:
            r["momentum"] = "Muy alto"
        elif strength >= 3:
            r["momentum"] = "Alto"
    ...
    for r in selected:
        ...
        # compatibilidad si algún handler mira este campo
        r["priceChangePercent"] = r["change_pct"]
