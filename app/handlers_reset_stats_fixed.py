...
async def handle_reset_stats(query, user):
    ...
    try:
        if callable(reset_statistics):
            reset_statistics()
        await query.edit_message_text(
            "♻️ Estadísticas restablecidas correctamente.\n\nSe reiniciaron desde cero las señales base, las señales de usuario y los resultados evaluados.",
            reply_markup=InlineKeyboardMarkup([
...
def get_handlers():
    return [
        CallbackQueryHandler(
            handle_menu,
            pattern="^(view_signals|radar|radar_refresh|performance|reset_stats|movers|market|market_refresh|watchlist|wl_refresh|wl_clear|wl_rm:[A-Z0-9]+|alerts|history|plans|my_account|referrals|support|admin_panel|admin_activate_plan|back_menu|choose_plus_plan|choose_premium_plan|register_exchange)$"
        ),
