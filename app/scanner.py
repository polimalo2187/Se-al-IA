...
# Umbrales por plan
PREMIUM_SCORE_MIN = float(os.getenv("PREMIUM_SCORE_MIN", "88"))
PLUS_SCORE_MIN = float(os.getenv("PLUS_SCORE_MIN", "80"))
FREE_SCORE_MIN = float(os.getenv("FREE_SCORE_MIN", "72"))
...
async def scan_market_async(bot: Bot):
    ...
    while True:
        try:
            ...
            # Enviamos máximo 3, pero clasificados por score, no por posición
            ...
            for signal in selected:
                ...
                visibility, medal = _classify_plan_by_score(score)

                # Si el score no da para free, no se envía
                if not visibility:
                    continue
