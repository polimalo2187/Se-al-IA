    ...
    while True:
        try:
            ...
            for idx, signal in enumerate(top_n):
                ...
                strategy_score = float(signal.get("score", 0))
                final_score = float(signal.get("final_score", strategy_score))

                # Misma lógica de setup para PREMIUM y PLUS.
                # La diferencia operativa/comercial aquí queda en el score asignado por plan.
                ...
                logger.info(
                    "✅ %s | %s %s | assigned_score=%s | strategy_score=%s | final_score=%s | entry_q=%s | vol_q=%s | plan=%s",
                    medal,
                    symbol,
                    direction,
                    score,
