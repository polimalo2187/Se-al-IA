...
#
# Esto evita que FREE herede setups shared y evita que PLUS
# reciba simplemente el remanente después de PREMIUM.
# ------------------------------------------------------
...
def _build_candidate(symbol: str, result: Dict, df_5m: pd.DataFrame) -> Dict:
    ...
    final_score = round(
        raw_score + (entry_quality * 0.35) + (volume_quality * 0.40),
        2,
    )
...
async def scan_market_async(bot: Bot):
    logger.info(
        "📡 Scanner iniciado — sin bloqueo global de Telegram + clasificación exclusiva por plan"
    )
